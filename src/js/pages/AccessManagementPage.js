 ...
 * Strict Department-Segregated Access & Download Requests Console
 * Exclusively handles:
...
export function renderAccessManagementPage(filters = {}) {
  ...
  if (!user) {
    return `<div class="content-container"><p>Please sign in to view Access Management.</p></div>`;
  }
 ...
 * Clean UI: 2 buttons, divider, My Requests table strictly for the user
 */
  ...
  return `
    <div class="content-container" style="max-width: 1100px;">
      ...
      <div class="page-header" style="margin-bottom: 24px;">
        <div>
          <h1 class="page-title" style="font-size: 24px; font-weight: 800; color: #0F172A; letter-spacing: -0.02em;">
            ACCESS MANAGEMENT
          </h1>
          <p class="page-subtitle" style="font-size: 13.5px; color: #64748B; margin-top: 4px;">
            Department: <strong style="color: #0F172A;">${dept.name}</strong> • Request access to your department database or permission to download department files.
          </p>
        </div>

        <!-- 2 Primary Action Buttons -->
        <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
          <button class="btn btn-primary" id="btn-request-db-access" onclick="window.openRequestDbAccessModal()" style="font-size: 13px; padding: 9px 16px; font-weight: 600; display: flex; align-items: center; gap: 6px;">
            <span>🗄️</span>
            <span>Request Database Access</span>
          </button>
          <button class="btn btn-secondary" id="btn-request-doc-download" onclick="window.openRequestDocDownloadModal()" style="font-size: 13px; padding: 9px 16px; font-weight: 600; display: flex; align-items: center; gap: 6px;">
            <span>📥</span>
      ...
      <!-- My Requests Section -->
      <div class="enterprise-card" style="margin-top: 8px;">
        <div class="enterprise-card-header" style="padding: 16px 20px; border-bottom: 1px solid #F1F5F9; display: flex; justify-content: space-between; align-items: center;">
          <div style="font-size: 15px; font-weight: 700; color: #0F172A;">
            My Requests (${requests.length})
          </div>
          <div style="font-size: 12px; color: #64748B;">
            Strictly segregated to ${dept.name} Department
          </div>
        </div>
          ...
          <table class="data-table" style="width: 100%; border-collapse: collapse;">
            <thead>
              <tr style="background: #F8FAFC; border-bottom: 1px solid #E2E8F0; text-align: left;">
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">Request ID</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">Type</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">Department</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">Item</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">Date</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">Status</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B; text-align: right;">Action</th>
              </tr>
            </thead>
            <tbody>
      ...
      <tr>
        <td colspan="7" style="text-align: center; padding: 48px 16px; color: #94A3B8;">
          ...
          <div style="font-size: 12px; color: #94A3B8; margin-top: 4px;">
            Use the buttons above to request access to your department database or permission to download documents.
          </div>
        </td>
      </tr>
    `;
    ...
    return `
      <tr style="border-bottom: 1px solid #F1F5F9;">
        ...
        <td style="padding: 12px 16px;">
          <span class="badge ${r.requestType === ACCESS_REQUEST_TYPES.DATABASE_ACCESS ? 'badge-purple' : 'badge-blue'}" style="font-size: 11px;">
            ${r.requestType === ACCESS_REQUEST_TYPES.DATABASE_ACCESS ? '🗄️ Database Access' : '📥 Document Download'}
          </span>
        </td>
...
function renderStatusBadge(status, denialReason = null) {
  if (status === ACCESS_STATUSES.APPROVED) {
    return `<span style="display: inline-flex; align-items: center; gap: 5px; font-size: 12px; font-weight: 700; color: #16A34A; background: #DCFCE7; padding: 4px 10px; border-radius: 9999px;">● Approved</span>`;
  }
  if (status === ACCESS_STATUSES.DENIED) {
    return `
      <div>
        <span style="display: inline-flex; align-items: center; gap: 5px; font-size: 12px; font-weight: 700; color: #DC2626; background: #FEE2E2; padding: 4px 10px; border-radius: 9999px;">● Denied</span>
        ${denialReason ? `<div style="font-size: 11px; color: #DC2626; margin-top: 4px;">${denialReason}</div>` : ''}
      </div>
    `;
  }
  return `<span style="display: inline-flex; align-items: center; gap: 5px; font-size: 12px; font-weight: 700; color: #D97706; background: #FEF3C7; padding: 4px 10px; border-radius: 9999px;">● Pending</span>`;
}

function renderUserRowAction(r) {
  if (r.status === ACCESS_STATUSES.APPROVED) {
    if (r.requestType === ACCESS_REQUEST_TYPES.DOCUMENT_DOWNLOAD) {
      return `
        <button class="btn btn-primary btn-sm" onclick="window.userDownloadApprovedDoc('${r.id}')" style="font-size: 11.5px; padding: 4px 10px;">
          ⬇️ Download Document
        </button>
      `;
    } else {
      return `
        <a href="#/departments" class="btn btn-secondary btn-sm" style="font-size: 11.5px; padding: 4px 10px; text-decoration: none;">
          🗄️ Open Database
        </a>
      `;
    }
  }
  if (r.status === ACCESS_STATUSES.DENIED) {
    return `<span style="font-size: 11.5px; color: #94A3B8;">Access Denied</span>`;
  }
  return `<span style="font-size: 11.5px; color: #D97706; font-style: italic;">Pending Legal Review</span>`;
}

/**
 * 2. Legal Admin Approval View
 * Features: 11 Department cards with pending badges, department filter, type filter, status filter, and Approve/Deny actions.
 */
      ...
      <div class="page-header" style="margin-bottom: 20px;">
        <div>
          ...
          <div class="page-subtitle" style="font-size: 13.5px; color: #64748B; margin-top: 4px;">
            Central Legal Approval Authority • Review and authorize database access and document downloads with strict department segregation.
          </div>
        </div>
      ...
      ">
        ...
        <div style="display: flex; align-items: center; gap: 8px;">
          ...
          <select 
            ...
            class="form-select" 
            style="font-size: 12.5px; height: 34px; padding: 4px 10px; min-width: 160px;"
            onchange="window.adminApplyFilters()"
          >
        ...
        <div style="overflow-x: auto;">
          <table class="data-table" style="width: 100%; border-collapse: collapse;">
            <thead>
              <tr style="background: #F8FAFC; border-bottom: 1px solid #E2E8F0; text-align: left;">
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">Request ID</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">User</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">Department</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">Request Type</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">Item</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">Date</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B;">Status</th>
                <th style="padding: 12px 16px; font-size: 12px; font-weight: 600; color: #64748B; text-align: right;">Action</th>
...
function renderAdminRowActions(r) {
  ...
  if (r.status === ACCESS_STATUSES.APPROVED) {
    return `<span style="font-size: 11.5px; color: #16A34A; font-weight: 600;">Authorized</span>`;
  }
  return `<span style="font-size: 11.5px; color: #DC2626; font-weight: 600;">Denied</span>`;
}

/**
 * Request Database Access Modal Form HTML
 * Strictly locks department to the authenticated user's department
 */
    ...
    <div>
      ...
      <div class="form-group" style="margin-bottom: 14px;">
        ...
        <div style="font-size: 11.5px; color: #64748B; margin-top: 4px;">
          🔒 Department is automatically assigned from your authenticated profile.
        </div>
      </div>
...
/**
 * Request Document Download Modal Form HTML
  ...
  return `
    <div>
      ...
      <div class="form-group" style="margin-bottom: 14px;">
        ...
        />
        <div style="font-size: 11.5px; color: #64748B; margin-top: 4px;">
          🔒 You may only request permission to download documents belonging to ${dept.name}.
        </div>
      </div>
        ...
        ${
          deptDocs.length === 0
            ...
            <div style="padding: 12px; background: #FFFBEB; border: 1px solid #FDE68A; border-radius: 8px; font-size: 12.5px; color: #92400E;">
              No documents currently found under the <strong>${dept.name}</strong> department repository.
            </div>
          `
            : `
      ...
      <div class="form-group" style="margin-bottom: 18px;">
        ...
        <textarea 
          ...
          class="form-textarea" 
          rows="3" 
          placeholder="Please explain the business justification for downloading this document..." 
          required 
          style="font-size: 13px; line-height: 1.5;"
        ></textarea>
      </div>
  ...
  return `
    <div>
      <p style="font-size: 13.5px; color: #334155; margin-bottom: 14px;">
        You are denying access request <strong>${requestId}</strong>. You may provide an optional reason below to notify the user.
      </p>
